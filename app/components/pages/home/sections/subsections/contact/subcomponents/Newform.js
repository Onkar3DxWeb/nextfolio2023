...
export default function Form() {
  ...
  const sendEmail = async(e) => {
    ...
    let formData = {
      ...
      try {
        ...
        emailjs.sendForm('service_rnuf9u8', 'template_07dkgpk', formRef.current, 'b9IPWOAd2xCebHu5l')
  ...
  return (
    <form ref={formRef} className="space-y-7 [&>div]:form-field-wrapper sm:space-y-9 xl:grid xl:grid-cols-2 xl:gap-6 xl:space-y-0 2xl:gap-10 3xl:gap-12 dpr-lg:gap-8 dpr-xl:gap-5" onSubmit={sendEmail}>
      <div className="xl:col-span-1">
        <label htmlFor="name">Name</label>
        <input ref={nameRef} type="text" id="name" name="from_name" autoComplete="off" />
      </div>
      <div className="xl:col-span-1">
        <label htmlFor="email">Email</label>
        <input ref={emailRef} type="email" id="email" name="email_id" autoComplete="off" />
      </div>
      <div className="xl:col-span-2">
        <label htmlFor="message" >Message</label>
        <textarea className="resize-none h-[20vh]" name="message" ref={messageRef} id="message" />
      </div>
      <button
